...
public final class DemoBoundedBuffer {
    ...
    private static void timeout() throws InterruptedException{
        ...
        for (int i = 0; i < nPros; i++) {
            producers[i] = new Producer(queue, random.nextInt(100));
        ...
        for (int i = 0; i < nPros; i++) {
            ...
            LOG.info("Prod " + (char) (i + 65) + " = " + producers[i].getTimedOut() + " timed out");
    ...
    private static void oldMain() throws InterruptedException{
        final Random random = new Random();
        final int nPros = 3;
        final Producer[] producers = new Producer[nPros];
        final ThreadGroup prosGroup = new ThreadGroup("Producer-Threads");
