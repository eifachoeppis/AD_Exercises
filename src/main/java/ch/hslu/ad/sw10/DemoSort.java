...
public final class DemoSort {
    ...
    public static void main(String[] args) {
        ...
        char[] randomChars = initializeRandomChars();
        ...
        */
        ...
        System.out.println("char quickSort, random: " + measure(randomChars, Sort::quickSort) + "ms");
        System.out.println("int quickSort, random: " + measure(random, Sort::quickSort) + "ms");
    }

    private static char[] initializeRandomChars() {
        char[] numbers = new char[SIZE];
        for (int i = 0; i < SIZE; i++) {
            numbers[i] = (char)RANDOM.nextInt(65536);
        }
        return numbers;
    }

    private static double measure(char[] numbers, Function<char[], char[]> sortFunction){
        ArrayList<Long> measurements = new ArrayList<>();

        for (int i = 0; i < ITERATIONS; i++) {
            long start = System.nanoTime();
            char[] sorted = sortFunction.apply(numbers);
